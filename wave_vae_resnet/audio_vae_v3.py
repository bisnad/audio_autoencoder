"""
same as audio_cae_v2.py but with variational loss from 
https://github.com/GuitarsAI/MLfAS/blob/master/MLAS_08_Variational_Autoencoder.ipynb
"""
...
load_weights = True
save_weights = True
encoder_weights_file = "results_v4/weights/encoder_weights_epoch_200"
decoder_weights_file = "results_v4/weights/decoder_weights_epoch_200"
...
ae_kld_loss_scale_max = 0.5 # 0.1
ae_kld_loss_incr = 0.005 # 0.0026
...
def reparameterize(mu, log_var):
    return mu + log_var*torch.randn_like(log_var)
...
def ae_loss(target_audio, pred_audio, mu, log_var):
    ...
    _ae_kld_loss = variational_loss2(mu, log_var)
...
def encode_sequences(waveform, frame_indices):
    ...
    for excerpt_index in range(seq_excerpt_count):
        ...
        excerpt_end_frame = excerpt_start_frame + audio_window_length
        excerpt = waveform[:, excerpt_start_frame:excerpt_end_frame]
        
        excerpt = np.expand_dims(excerpt, axis=0)
        excerpt = torch.from_numpy(excerpt).reshape(1, audio_channels, audio_window_length)
        
        if audio_standardise:
            excerpt = (excerpt - full_dataset.audio_mean) / full_dataset.audio_std
            
        excerpt = excerpt.to(device)

        with torch.no_grad():
            encoder_output = audio_encoder(excerpt)
            
        encoder_output_mu = encoder_output[:, encoder_output.shape[1] // 2:, :]
        encoder_output_log_var = encoder_output[:, :encoder_output.shape[1] // 2, :]
        latent_vector = reparameterize(encoder_output_mu, encoder_output_log_var)
...
def decode_sequences(sequence_encodings, file_name):

    grain_env = torch.hann_window(audio_window_length)
    grain_offset = audio_window_length // 2
    
    predict_window_count = len(sequence_encodings)
...
start_seq_index = test_start_time_sec * audio_sample_rate 
end_seq_index = start_seq_index + test_duration_sec * audio_sample_rate

seq_indices = [ frame_index for frame_index in range(start_seq_index, end_seq_index, grain_offset)]

seq_encodings = encode_sequences(test_waveform, seq_indices)
decode_sequences(seq_encodings, "test.wav")
...
start_seq_index = test_start_time_sec * audio_sample_rate 
seq_frame_count = test_duration_sec * audio_sample_rate // (audio_window_length - grain_offset)
...
seq_encodings[0].shape

for index in range(0, seq_frame_count - 1):
    random_step = np.random.random((seq_encodings[0].shape[0], seq_encodings[0].shape[1])).astype(np.float32) * 0.001
    seq_encodings.append(seq_encodings[index] + random_step)

decode_sequences(seq_encodings, "rand.wav")
...
for index in range(len(seq_encodings)):
    ...
    offset = np.ones(shape=(seq_encodings[0].shape[0], seq_encodings[0].shape[1]), dtype=np.float32) * sin_value * 0.1
    
    offset_seq_encoding = seq_encodings[index] + offset
    offset_seq_encodings.append(offset_seq_encoding)
    
decode_sequences(offset_seq_encodings, "offset.wav")

# interpolate two original sequences

seq1_start_time_sec = 20
seq2_start_time_sec = 100
seq_duration_sec = 20

seq1_start_index = seq1_start_time_sec * audio_sample_rate
seq1_end_index = seq1_start_index + seq_duration_sec * audio_sample_rate

seq2_start_index = seq2_start_time_sec * audio_sample_rate
seq2_end_index = seq2_start_index + seq_duration_sec * audio_sample_rate

seq1_indices = [ frame_index for frame_index in range(seq1_start_index, seq1_end_index, grain_offset)]
seq2_indices = [ frame_index for frame_index in range(seq2_start_index, seq2_end_index, grain_offset)]

seq1_encodings = encode_sequences(test_waveform, seq1_indices)
seq2_encodings = encode_sequences(test_waveform, seq2_indices)
...
decode_sequences(mix_encodings, "mix.wav")

# extrapolate two original sequences
...
for index in range(len(seq1_encodings)):
    mix_factor = index / (len(seq1_indices) - 1)
    mix_factor = mix_min + mix_factor * (mix_max - mix_min)
    
    mix_encoding = seq1_encodings[index] * (1.0 - mix_factor) + seq2_encodings[index] * mix_factor
    mix_encodings.append(mix_encoding)

decode_sequences(mix_encodings, "mix2.wav")
