...
load_weights = True
save_weights = True
encoder_weights_file = "results_no_kld_vl_v1/weights/encoder_weights_epoch_200"
decoder_weights_file = "results_no_kld_vl_v1/weights/decoder_weights_epoch_200"
...
ae_kld_loss_scale_max = 0.1 # 0.1
ae_kld_loss_incr = 0.0026 # 0.0026
...
def ae_loss(target_audio, pred_audio, mu, log_var):
    ...
    # ldk loss
    _ae_kld_loss = variational_loss(mu, log_var)
...
def ae_train_step(target_audio, epoch):
    ...
    encoder_output_log_var2 = torch.abs(encoder_output_log_var) + 0.00001

    decoder_input = reparameterize(encoder_output_mu, encoder_output_log_var2)
    
    #decoder_input = encoder_output_mu
    
    pred_audio = audio_decoder(decoder_input)
    
    _ae_loss, _ae_rec_loss, _ae_kld_loss = ae_loss(target_audio, pred_audio, encoder_output_mu, encoder_output_log_var2) 
...
def create_pred_sonification(waveform, file_name):
    ...
    for i in range(predict_window_count - 1):
        ...
        with torch.no_grad():
            ...
            encoder_output_log_var2 = torch.abs(encoder_output_log_var)
